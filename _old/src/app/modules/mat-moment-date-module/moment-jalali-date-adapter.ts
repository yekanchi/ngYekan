...
import {NumberHelper} from "./numnber-helper";
...
export class MomentJalaliDateAdapter extends DateAdapter<moment.Moment> {
	// Note: all the methods that accept a `Moment` input parameter immediately call `this.clone`
	 ...
	 * Calendar type
	 */
	private calendarType!: 'Gregorian' | 'Jalaali';
	// | 'Hijri';
	...
	getYearName(date: moment.Moment): string {
		switch (this.calendarType) {
			...
			case 'Jalaali':
				return NumberHelper.toPersianNumber(this.clone(date).jYear());
			// case 'Hijri':
	...
	getNumDaysInMonth(
		date: moment.Moment | momentJalaali.Moment
		// | momentHijri.Moment
	): number {
		switch (this.calendarType) {
			case 'Gregorian':
	...
	parse(value: any, parseFormat: any): moment.Moment | null {
		if (value && typeof value === 'string') {
			value = NumberHelper.toEngNumber(value);
			switch (this.calendarType) {
				case 'Gregorian':
